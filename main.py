...
# База пользователей: { "@id": {"name": "Имя", "avatar": "data:image..."} }
registered_users = {}
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        /* Окно регистрации */
        #authModal { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: #0f172a; z-index: 1000; display: none; flex-direction: column; align-items: center; justify-content: center; padding: 20px; }
    ...
    <script>
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            if (savedProfile) {
                ...
                // Регистрируем профиль на сервере
        ...
        function startNewChat() {
            ...
            socket.emit('find_user', { id: searchId }, (response) => {
                if(response.found) {
                    ...
                    if(!chatsData[target.name]) { chatsData[target.name] = { avatar: target.avatar, msgs: [] }; }
                    document.getElementById('newChatInput').value = '';
                    renderChatsList(); openChat(target.name);
        ...
        function renderChatsList() {
            ...
            keys.forEach(name => {
                ...
                const lastMsg = chat.msgs.length > 0 ? (chat.msgs[chat.msgs.length - 1].type === 'text' ? chat.msgs[chat.msgs.length - 1].text : '[Медиафайл]') : "Нет сообщений";
                const avatarHtml = chat.avatar ? `<img src="${chat.avatar}" class="avatar">` : `<div class="avatar">${name[0].toUpperCase()}</div>`;
        ...
        function openChat(name) {
            ...
            document.getElementById('headerLeft').innerHTML = `<button class="back-btn" onclick="goBackToChats()">⬅ Назад</button><div style="font-weight: bold; font-size: 15px; margin-left: 5px;">${name}</div>`;
        }

        function goBackToChats() {
            document.getElementById('headerLeft').innerHTML = `<div id="headerAvatarBox" class="avatar"></div><div><div id="headerName" style="font-weight: bold; font-size: 15px;">Чаты</div><div class="status-text"><span class="status-dot online"></span><span id="headerSubtext">В сети</span></div></div>`;
        ...
        function sendMessage(e) {
            ...
            const msgData = { chat: currentChatUser, user: myProfile.name, text: text, type: 'text', time: timeStr };
        ...
        function sendSticker(stickerEmoji) {
            ...
            const msgData = { chat: currentChatUser, user: myProfile.name, text: stickerEmoji, type: 'sticker', time: timeStr };
            socket.emit('send_message', msgData);
        ...
        function sendMediaFile(input, type) {
            ...
            if(file && currentChatUser) {
                ...
                reader.onload = function(evt) {
                    ...
                    const msgData = { chat: currentChatUser, user: myProfile.name, media: evt.target.result, type: type, time: timeStr };
                    socket.emit('send_message', msgData);
        ...
        socket.on('receive_message', (data) => {
            if(data.user !== myProfile.name) {
                if(!chatsData[data.user]) chatsData[data.user] = { avatar: "", msgs: [] };
