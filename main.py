...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: sans-serif; }
        ...
        /* СТИЛИ ЭКРАНА VPN */
        .vpn-container { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 25px 20px; overflow-y: auto; }
        .cat-status { text-align: center; margin-top: 10px; }
        .cat-avatar { font-size: 70px; margin-bottom: 10px; transition: transform 0.3s ease; }
        .status-title { font-size: 18px; font-weight: bold; color: white; }
        .status-sub { font-size: 13px; color: #94a3b8; margin-top: 4px; }
        .power-btn-container { position: relative; display: flex; align-items: center; justify-content: center; margin: 20px 0; }
        .power-btn {
            width: 140px; height: 140px; border-radius: 50%; background: #1e293b; border: 3px solid #334155;
            color: #64748b; font-size: 45px; cursor: pointer; display: flex; align-items: center; justify-content: center;
            box-shadow: 0 8px 25px rgba(0, 0, 0, 0.4); transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); outline: none;
        }
        ...
        .settings-card { width: 100%; max-width: 360px; background: #1e293b; border: 1px solid #334155; border-radius: 16px; padding: 14px 16px; margin-bottom: 10px; }
...
<body>
    ...
    <div class="content-area">
        ...
        <!-- ЭКРАН ЧАТА С ПОЛЬЗОВАТЕЛЕМ/БОТОМ -->
        ...
        <div id="screenVPN" class="screen">
            <div class="vpn-container">
                <div class="cat-status">
                    ...
                    <div class="status-sub" id="statusSub">Нажмите кнопку для подключения</div>
                </div>
                    ...
                    <div class="settings-card">
                        <div class="card-row">
                            <div class="card-info">
                                ...
                                <div>
                                    <div class="card-title">Германия (Франкфурт)</div>
                                    <div class="card-desc">VLESS-Reality • Низкий пинг</div>
                                </div>
                            </div>
                    ...
                    <div class="settings-card">
                        <div class="card-row">
                            <div class="card-info">
                                <span style="font-size: 22px;">🎮</span>
                                <div>
                                    <div class="card-title">Roblox Ultra-Turbo</div>
                                    <div class="card-desc">Быстрый запуск и обход</div>
                                </div>
                            </div>
                            <label class="switch">
    ...
    <script>
        ...
        function toggleVPN() {
            ...
            if (isVpnConnected) {
                ...
                title.innerText = 'Cat VPN Включен';
                title.style.color = '#22c55e';
                sub.innerText = 'Защита активна • Roblox готов к игре';
            } else {
                ...
                sub.innerText = 'Нажмите кнопку для подключения';
            }
        }
    ...
    if target_chat == BOT_NAME or target_chat == BOT_ID:
        ...
        try:
            if not GEMINI_API_KEY:
                bot_reply = "Мяу! Переменная GEMINI_API_KEY не найдена в Environment Variables."
            else:
                response = model.generate_content(user_text)
                bot_reply = response.text
        except Exception as e:
            bot_reply = f"Ошибка Gemini API: {str(e)}"
