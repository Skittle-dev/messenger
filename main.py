...
<body>
        ...
        <div id="screenProfile" class="screen">
            <div class="profile-card">
                ...
                <div class="field-group">
                    ...
                    <!-- ПЛАШКА СТАТУСА ПОЧТЫ -->
                    <div id="emailStatusBadge" style="margin-top: 6px; font-size: 12px; font-weight: bold;"></div>
                </div>
    ...
    <script>
        ...
        function requestVerificationCode(mode) {
            ...
            socket.emit('send_verify_code', { email: email }, (res) => {
                if(res.status === 'ok') {
                    ...
                    alert("Код отправлен! Проверьте вашу почту или логи сервера.");
        ...
        function updateEmailBadge() {
            ...
            if (myProfile.email && myProfile.email.includes('@')) {
                badge.innerHTML = '<span style="color:#22c55e;">✓ Почта привязана</span>';
            } else {
                badge.innerHTML = '<span style="color:#ef4444;">❌ Почта не привязана</span>';
            }
        }
        ...
        function logoutUser() {
            if (confirm("Выйти из аккаунта?")) { localStorage.clear(); location.reload(); }
        }
        ...
        function renderChatsList() {
            ...
            if(keys.length === 0) { c.innerHTML = `<div class="empty-chats">📭 У вас пока нет чатов</div>`; return; }
            keys.forEach(name => {
...
def handle_send_verify(data):
    ...
    try:
        # Отправка писем через API Resend
        resend.Emails.send({
            "from": "Cat Bot <onboarding@resend.dev>",
            "to": recipient,
            "subject": "Код подтверждения Cat Messanger",
            "html": f"<p>Ваш код подтверждения: <strong>{code}</strong></p>"
        })
