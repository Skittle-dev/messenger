...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        #verifyModal, #authModal { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: #0f172a; z-index: 1000; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 20px; }
...
<body>
    ...
    <div id="verifyModal">
        <div class="auth-box">
            ...
            <div id="stepEmail">
                ...
                <button class="btn-primary" style="width: 100%;" onclick="requestVerificationCode()">Отправить код</button>
            ...
            <div id="stepCode" style="display:none;">
                ...
                <button class="btn-primary" style="width: 100%;" onclick="verifyCode()">Подтвердить</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // 1. Восстанавливаем профиль И сохраненные чаты
        window.addEventListener('DOMContentLoaded', () => {
            ...
            if (savedProfile) {
                ...
                // Если не зарегистрирован, показываем окно верификации бота
        ...
        function requestVerificationCode() {
            ...
            socket.emit('send_verify_code', { email: email }, (response) => {
                if(response.status === 'ok') {
                    ...
                    alert("Код отправлен! Проверьте входящие и папку Спам.");
                } else {
        ...
        function verifyCode() {
            ...
            socket.emit('check_verify_code', { email: verifiedEmail, code: code }, (response) => {
                if(response.status === 'ok') {
                    document.getElementById('verifyModal').style.display = 'none';
...
# БОТ: Генерация и отправка кода на почту
...
# БОТ: Проверка введенного пользователем кода
...
def handle_check_verify(data):
  ...
  if recipient in verification_codes and verification_codes[recipient] == code:
    del verification_codes[recipient]  # Удаляем использованный код
    return {'status': 'ok'}
  return {'status': 'error'}
