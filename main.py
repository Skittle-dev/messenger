...
def cat_proxy():
    ...
    try:
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) CatMessengerProxy/1.0'}
        resp = requests.get(target_url, headers=headers, timeout=10)
        
        content_type = resp.headers.get('Content-Type', '')

        # Обработка Cat Lite (Сжатие картинок)
        if lite_mode and 'image' in content_type:
            try:
                img = Image.open(io.BytesIO(resp.content))
                img.thumbnail((400, 400))  # Уменьшаем разрешение
                out_io = io.BytesIO()
                img.save(out_io, format='JPEG', quality=40)  # Сжимаем качество
                out_io.seek(0)
                return Response(out_io.getvalue(), mimetype='image/jpeg')
            except Exception:
                pass

        # Обработка HTML для замещения ссылок в режиме Cat Lite
        if 'text/html' in content_type and lite_mode:
            soup = BeautifulSoup(resp.text, 'html.parser')
            # Удаляем рекламные скрипты и тяжелые элементы
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: sans-serif; }
        ...
        .cat-status { text-align: center; margin-top: 5px; }
        .cat-avatar { font-size: 60px; margin-bottom: 5px; transition: transform 0.3s ease; }
        ...
        .power-btn {
            width: 120px; height: 120px; border-radius: 50%; background: #1e293b; border: 3px solid #334155;
        ...
        .settings-card { width: 100%; max-width: 360px; background: #1e293b; border: 1px solid #334155; border-radius: 16px; padding: 12px 14px; margin-bottom: 10px; }
        ...
        .proxy-browser input { width: 100%; background: #0f172a; border: 1px solid #334155; padding: 10px; border-radius: 10px; color: white; outline: none; }
        #proxyFrame { width: 100%; height: 250px; border: 1px solid #334155; border-radius: 12px; background: white; display: none; }
    </style>
</head>
<body>
        ...
        <div id="screenVPN" class="screen">
            <div class="vpn-container">
                ...
                <div style="width: 100%; max-width: 360px;">
                        ...
                        <div class="card-row">
                            <div class="card-info">
                                ...
                                <div>
                                    ...
                                    <div class="card-desc">Сжатие медиа и очистка рекламы</div>
                                </div>
                            </div>
                            <label class="switch">
                    ...
                    <!-- Поле открытия сайтов через VPN-прокси -->
                    <div class="proxy-browser">
                        <div style="display: flex; gap: 6px;">
                            <input type="text" id="proxyUrl" placeholder="Введите сайт (например, wikipedia.org)...">
